...
class VAE(Model):
    ...
    def reparameterize(self, mean, logvar, device):
        with tf.device(device):
    ...
    def sample(self, device, rand_vec=None):
        with tf.device(device):
            if rand_vec == None:
                rand_vec = random.normal(shape=(100, self.latent_dim))
            return tf.sigmoid(self.decode(rand_vec, device)) # binarize the result for visibility


'''Define loss function and training proceedures'''

def log_normal(sample, mean, logvar, device):
    with tf.device(device):
        prob = -0.5 * ((sample - mean) ** 2.0 * tf.exp(-logvar) + logvar + tf.math.log(2.0 * np.pi))
        return tf.reduce_sum(prob, axis=1)

def get_loss(model, x, device):
    with tf.device(device):
        mean, logvar = model.encode(x, device) # defining q(z|x)
        z = model.reparameterize(mean, logvar, device) # sampled from q(z|x)
        xhat = model.decode(z, device) # generated by obtaining p(x|z)
        ...
        logpz = log_normal(z, mean, logvar, device) # standard Gaussian (assumed)
        logqzx = log_normal(z, mean, logvar, device) # Gaussian obtained through encoder

        # return the average value for each sample within this batch
        return -tf.reduce_mean(logpxz + logpz - logqzx)

'''use Adan'''
def train_per_batch(model, x, device):
    optimizer = optimizers.Adam(learning_rate=1e-4)
    with tf.device(device):
        with tf.GradientTape() as gt:
            loss = get_loss(model, x, device)
            gradient = gt.gradient(loss, model.trainable_variables)
            optimizer.apply_gradients(zip(gradient, model.trainable_variables))
        return loss

def train(model, device, epochs, x, x_test=None):
    # for each epoch
    with tf.device(device):
        for epoch in range(epochs):
            ...
            for x_batch in x:
                train_loss(train_per_batch(model, x_batch, device))
            ...
            if x_test is not None:
                for test_batch in x_test:
                    ...
                    break
                pred = predict(model, x_sample, device)
                show_pred(pred, epoch)

def predict(model, x, device):
    with tf.device(device):
        mean, logvar = model.encode(x, device) 
        z = model.reparameterize(mean, logvar, device) 
        return model.sample(device, z) 
